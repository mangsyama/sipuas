...
const isAdmin = computed(() => {
    ...
    return Number(user.role_id) === 1 || String(user.role).toUpperCase() === 'ADMINISTRATOR';
});
...
const submitDeleteReport = () => {
    if (!reportToDelete.value || isDeleting.value) return;
    isDeleting.value = true;

    router.delete(route('kasi.reports.destroy', { id: reportToDelete.value.id }), {
        preserveScroll: true,
        onFinish: () => {
...
<template>
    ...
    <AuthenticatedLayout>
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <Link 
                                        ...
                                        class="font-bold text-slate-900 dark:text-white text-xs hover:text-emerald-600 dark:hover:text-emerald-400 hover:underline transition-colors block"
                                    >
                                        {{ item.id }}
                                    </Link>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <Link
                                            :href="route('kasi.verify', { id: item.id })"
                                            :class="[
                                                'w-28 h-9 rounded-xl text-xs font-bold inline-flex items-center justify-center gap-1.5 transition-all duration-150 border',
                                                item.status === 'PENDING'
                                        ...
                                        <button
                                            ...
                                            type="button"
                                            class="h-9 w-9 rounded-xl border border-rose-200/80 dark:border-rose-900/60 bg-rose-50/80 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 inline-flex items-center justify-center transition cursor-pointer shrink-0 shadow-xs"
                                            title="Hapus Laporan (Soft Delete - Administrator)"
                                        >
                        ...
                        >
                            <div class="space-y-3">
                                <div class="flex items-center justify-between gap-2">
                                    <div>
                                        <Link 
                                            :href="route('kasi.verify', { id: item.id })"
                                            class="text-xs font-black text-slate-900 dark:text-white hover:text-emerald-600 dark:hover:text-emerald-400 hover:underline transition-colors block"
                                        >
                                            {{ item.id }}
                                        </Link>
                            ...
                            <div class="flex items-center gap-2">
                                <Link
                                    :href="route('kasi.verify', { id: item.id })"
                                    :class="[
                                        'flex-1 h-10 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition',
                                        item.status === 'PENDING'
                                ...
                                <button
                                    ...
                                    @click.stop="confirmDelete(item)"
                                    type="button"
                                    class="h-10 w-10 rounded-xl border border-rose-200/80 dark:border-rose-900/60 bg-rose-50/80 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 inline-flex items-center justify-center transition cursor-pointer shrink-0 shadow-xs"
                                    title="Hapus Laporan (Soft Delete - Administrator)"
                                >
                                    <Trash2 class="h-4 w-4" />
                                </button>
        ...
        <!-- Modal Konfirmasi Hapus Laporan (Administrator Only - Standard Dialog Style) -->
        <Transition
        ...
        >
            <div v-if="showDeleteConfirmModal" class="fixed inset-0 z-50 overflow-y-auto p-4 sm:p-0 flex items-center justify-center min-h-screen font-['Poppins',sans-serif]">
                <div class="fixed inset-0 bg-slate-900/60 dark:bg-slate-950/80 transition-opacity select-none" @click="showDeleteConfirmModal = false; reportToDelete = null"></div>
                ...
                <div class="relative bg-white dark:bg-slate-900 rounded-2xl border-0 shadow-2xl w-full max-w-md p-6 pb-8 sm:pb-6 text-center transform transition-all space-y-4 z-10">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white">Hapus Laporan Aduan?</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Apakah Anda yakin ingin menghapus laporan <strong class="text-slate-700 dark:text-slate-200 font-mono">#{{ reportToDelete?.id }}</strong> (Pelapor: <strong class="text-slate-700 dark:text-slate-200">{{ reportToDelete?.reporter_name }}</strong>)? Tindakan ini akan mengarsipkan laporan (soft delete) dari sistem.
                        </p>
                    </div>
                    <div class="flex flex-col-reverse sm:flex-row items-center justify-center gap-2.5 pt-2 w-full">
                        <button
                            type="button"
                            :disabled="isDeleting"
                            @click="showDeleteConfirmModal = false; reportToDelete = null"
                            class="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer text-center justify-center disabled:opacity-50"
                        >
                            Batal
                        </button>
                        <button
                            type="button"
                            :disabled="isDeleting"
                            @click="submitDeleteReport"
                            class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white text-xs font-semibold transition cursor-pointer text-center justify-center inline-flex items-center gap-2 disabled:opacity-50"
                        >
                            <span v-if="isDeleting" class="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
